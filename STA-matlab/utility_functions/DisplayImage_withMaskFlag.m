...
function DisplayImage_withMaskFlag(im,EigInfo,ConvInfo,para,mask,varargin)
...
if mask_near_cells_flag
    ...
    mask_near_cells = mask_near_cells.*mask;
else 
...
P2short=kron([1;1],Centrey)+0.9*scl*kron([1;-1],a2(Cent).*mask_near_cells(Cent));
   ...
   for sI = 1:size(P1,2)
       %plot(P1(:,sI),P2(:,sI),'k','linewidth',para.width);%6*0.645/0.46); %Or for brainmaps: (6*0.65/0.46));% title('tensor orientation');hold off
       h = plot(P1short(:,sI),P2short(:,sI),'linewidth',para.width); %4*0.645/0.46);% title('tensor orientation');hold off
       h.Color = rgbTmp(sI,:);
   end
